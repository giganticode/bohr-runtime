...
def calculate_metrics_for_heuristic(task: Task, path_config: PathConfig) -> None:
    ...
    for dataset in tqdm(task.test_datasets, desc="Calculating metrics for dataset: "):
        ...
        heuristic_groups = get_heuristic_files(
            path_config.heuristics, task.top_artifact
        )
        for heuristic_group in heuristic_groups:
            ...
            if not(labeling_functions):
                raise AssertionError(f'No labeling functions in {heuristic_group}')
            heuristic_group = relative_to_safe(heuristic_group, path_config.heuristics)
